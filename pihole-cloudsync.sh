...
# Version 1.4 - July 10, 2019 - Steve Jenkins (stevejenkins.com)
...
push () {
	...
	cd $personal_git_dir || exit
        ...
        CHANGED=$($SUDO git remote update > /dev/null && git --work-tree=$personal_git_dir status --porcelain > /dev/null)
        if [ -n "${CHANGED}" ]; then
